...
public abstract class IpUtil {
    ...
    public static String getCityInfo(String ip) {
        ...
        try {
            String dbPath = System.getProperty("java.io.tmpdir") + "ip2region.db";
            File file = new File(dbPath);
            if (!file.exists()) {
                file = new File(dbPath);
                FileUtils.copyInputStreamToFile(Objects.requireNonNull(IpUtil.class.getClassLoader().getResourceAsStream("ip2region.db")), file);
            }
    ...
    private static String parseCityInfo(String region) {
        ...
        log.debug("解析IP地址开始:{}, 解析结果:{}", region, result);
        return result;
    }
